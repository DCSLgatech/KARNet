...
class AttentionConv(nn.Module):
    def __init__(self,
        ...
        assert self.out_channels % self.groups == 0, "out_channels should be divided by groups. (example: out_channels: 40, groups: 4)"  # noqa
    ...
    def forward(self, x):
        ...
        k_out = k_out.unfold(2, self.kernel_size,
                             self.stride).unfold(3, self.kernel_size,
                                                 self.stride)
        v_out = v_out.unfold(2, self.kernel_size,
                             self.stride).unfold(3, self.kernel_size,
                                                 self.stride)
        ...
        out = torch.einsum('bnchwk,bnchwk -> bnchw', out,
                           v_out).view(batch, -1, height, width)
...
class AugmentedConv(nn.Module):
    def __init__(self,
        ...
        if self.relative:
            self.key_rel_w = nn.Parameter(
                torch.randn((2 * self.shape - 1, dk // Nh),
                            requires_grad=True))
    ...
    def relative_logits(self, q):
        ...
        rel_logits_w = self.relative_logits_1d(q, self.key_rel_w, H, W, Nh,
                                               "w")
    ...
    def relative_logits_1d(self, q, rel_k, H, W, Nh, case):
        ...
        if case == "w":
            ...
            rel_logits = torch.transpose(rel_logits, 2,
                                         4).transpose(4, 5).transpose(3, 5)
